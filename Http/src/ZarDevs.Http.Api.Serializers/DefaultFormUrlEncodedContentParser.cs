...
{
    /// <summary>
    /// A parser interface that will try and take a <see cref="object"/> and cast it to various enumerables of <see cref="Tuple{T1, T2}"/> or <see cref="KeyValuePair{TKey, TValue}"/> where key and value is a string.
    /// </summary>
    ...
    /// <summary>
    /// A parser that will try and take a <see cref="object"/> and cast it to various enumerables of <see cref="Tuple{T1, T2}"/> or <see cref="KeyValuePair{TKey, TValue}"/> where key and value is a string.
    /// </summary>
    public class DefaultFormUrlEncodedContentParser : IDefaultFormUrlEncodedContentParser
    {
        ...
        public bool TryParse(object value, out IEnumerable<KeyValuePair<string, string>> parsed)
        {
            ...
            if (value is IEnumerable<(string key, string value)> v)
                parsed = v.Select(i => new KeyValuePair<string, string>(i.key, i.value)).ToArray();
